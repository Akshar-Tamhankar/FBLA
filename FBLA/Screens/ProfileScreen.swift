...
struct ProfileScreen: View {
    ...
    // Expect at most one profile; we take the first if multiple exist.
    ...
    var body: some View {
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        if let p = profile {
                            ToggleRow(
                                ...
                                isOn: Binding(
                                    get: { p.notifyAnnouncements },
                                    set: { p.notifyAnnouncements = $0; save() }
                                )
                            )
                            Divider().opacity(0.2)
                            ToggleRow(
                            ...
                            HStack(spacing: 12) {
                                ...
                                VStack(alignment: .leading, spacing: 2) {
                                    ...
                                    Text(membershipText(p.membershipExpiration))
                                        .font(.caption)
                                        .foregroundColor(.secondary)
                                }
                                Spacer()
                                Button {
                                    showEdit = true // edit in the same sheet
                                } label: {
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        if let p = profile, !p.badges.isEmpty {
                            FlowLayout(spacing: 8) {
                                ForEach(Array(p.badges.enumerated()), id: \.offset) { idx, badge in
                                    BadgeChip(badge.label)
                                        .contextMenu {
        ...
        .alert("Add Badge", isPresented: $showAddBadge, actions: {
            ...
            Text("Enter a short label, e.g., “Regionals 2026”.")
        })
    }
...
// Lightweight flow layout (as in your earlier version)
        ...
        NavigationView {
            Form {
                ...
                Section(header: Text("Service")) {
                    Stepper(
                        "Hours: \(Int(profile.serviceHours))",
